...
class AdvertiseDirector {
  ...
  static Future<bool> isIranianUser() async {
    try {
      ...
      if (isIran) {
        debugPrint(
          '🇮🇷 Iranian user detected (timezone: $currentTimeZone) - AdMob disabled',
        );
      }
      return isIran;
    } catch (e) {
  ...
  static Future<bool> shouldUseInternalAds(Ref ref) async {
    ...
    if (await isIranianUser()) {
      debugPrint(
        '📍 Ad Manager - Iranian user detected, using InternalAdStrategy only (AdMob disabled)',
      );
      return true;
    }
    ...
    if (Platform.isWindows || Platform.isMacOS || Platform.isLinux) {
      debugPrint(
        '📍 Ad Manager - Desktop platform detected, using InternalAdStrategy only',
      );
      return true;
    }
    ...
    debugPrint(
      '📍 Ad Manager - Mobile platform detected, using DUAL strategy approach',
    );
    ...
    if (adversies['api_advertise'] != null) {
      ...
      debugPrint(
        '📍 Ad Manager - Available ad keys: ${advertiseMap.keys.toList()}',
      );
    }
  ...
  static Future<Map<String, String>> getRandomCustomAd(Ref ref) async {
    ...
    if (adversies['api_advertise'] != null) {
      ...
      debugPrint(
        '📍 Ad Manager - Available keys: ${advertiseMap.keys.toList()}',
      );
    }
